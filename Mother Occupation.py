...
sumSix = 0
for i in range(145):
    if df["Mom occupation"][i] == 1:
        list_one.append(1)
        sumOne += df["GRADE"][i]
    elif df["Mom occupation"][i] == 2:
        list_two.append(2)
        sumTwo += df["GRADE"][i]
    elif df["Mom occupation"][i] == 3:
        list_three.append(3)
        sumThree += df["GRADE"][i]
    elif df["Mom occupation"][i] == 4:
        list_four.append(4)
        sumFour += df["GRADE"][i]
    else:
        list_five.append(5)
...
plt.xlabel("Mother: Retired, Gov. Officer, Private Sector Employee, Self-Employed, Other")
