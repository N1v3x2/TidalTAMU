...
print(df["Attendance to the seminars/conferences related to the department"])
list_one = []
sumOne = 0
list_two = []
sumTwo = 0
for i in range(145):
    if df["Attendance to the seminars/conferences related to the department"][i] == 1:
        list_one.append(1)
        sumOne += df["GRADE"][i]
    else:
        list_two.append(2)
...
plt.bar(data.keys(), data.values(), color='olive',)

plt.xlabel("Yes or No for attending seminar")
