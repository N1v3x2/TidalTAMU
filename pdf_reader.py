...
def read_grade_distributions(path):
    ...
    class_data_pattern_pdfminer = re.compile(
        ...
        # Non-greedy match for any characters until grade percentages
        r'.*?'
...
def grade_to_points(grade):
    grade_dict = {'A': 4.0, 'B': 3.0, 'C': 2.0,
                  'D': 1.0, 'F': 0.0, 'TCR': None}
...
def read_transcript(path):
    ...
    for match in re.findall(course_pattern, text):
        ...
        courses.append({'Subject': subj, 'Number': num,
                       'Title': title, 'Credit': float(cred), 'Grade': grade})
